...
-- 自定义split函数
local function stringSplit(inputStr, delimiter)
    local results = {}
    ...
    if (code ~= "") then
        ...
        if (preedit == code) then
            ...
            for i, v in ipairs(cands) do
                local paraphrase = v.paraphrase
                local splitResult = stringSplit(paraphrase, ";")
                for i, each in ipairs(splitResult) do
                    local result, _ = string.gsub(each, "[^a-zA-Z ]", "")
                    local c = Candidate("c2e", seg.start, seg._end, result, "〔" .. v.key .. "〕")
                    c.quality = 2
                    c.preedit = pinyin
                    yield(c)
                end
            ...
            --检查触发开关
            local flag = context:get_property("c2e")
            if (flag ~= nil and flag ~= "") then
                ...
                --发送请求
                local res = http.request("http://dict.iciba.com/dictionary/word/suggestion?word=" .. escape(text))
                local _, j = pcall(json.decode, res)
                if (j.status == 1 and j.message) then
                    cands = j.message
                end
                --生成候选项
                for i, v in ipairs(cands) do
