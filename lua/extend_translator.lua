	...
	if (code ~= "") then
		...
		if (preedit == code) then
			...
			for i, v in ipairs(cands) do
				local c = Candidate("extend", seg.start, seg._end, v.q, "〔E〕")
				c.quality = 2
				c.preedit = pinyin
				yield(c)
			end
			...
			--检查触发开关
			local flag = context:get_property("extend")
			if (flag ~= nil and flag ~= "") then
				...
				--发送请求
				local res = http.request("http://www.baidu.com/sugrec?prod=pc&wd="..escape(text).."&sugmode=2")
